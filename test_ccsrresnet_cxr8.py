...
def args_parse():
    ...
    parser.add_argument('--data', default="../datasets/miniCXR8/images/", help="Path to where data is stored")
    ...
    parser.add_argument('--savedir', default="output/ccsrresnet/", help="Path to store generated super resolution files")
...
def test(modelSR, dataloader):
    ...
    for i, (imageLR, imageHR) in enumerate(dataloader):
        ...
        for j in range(imageLR.size(0)):
            ...
            filepath = dataset.at(total_images + j).lstrip('../datasets/miniCXR8/images/')
            ...
            image = imageLR[j][0]
            image = image.cpu().data.numpy()
            filepath = 'noise_' + dataset.at(total_images + j).lstrip('../datasets/miniCXR8/images/')
            image = Image.fromarray((255 * image).astype(np.uint8))
            image.save(os.path.join(args.savedir, filepath))
