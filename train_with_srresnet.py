...
def train(model, srresnet, dataloader, scale_factor=2):
    model.train()
    ...
    for e in range(args.epochs):
        ...
        for i, (imageLR, imageHR) in enumerate(dataloader):
            ...
            srresnet_psnr += psnr(srresnetHR, imageHR).sum().item()

            if (i + 1) % 100 == 0:
                print("Norm residual learned: {}".format(torch.norm(learnedResidual.view(-1, 1 * M * N), dim=1).mean()))

            if (i + 1) % 25 == 0:
                print("Epoch [{} / {}]: Batch: [{} / {}]: Avg Training Loss: {:0.4f}, Avg Training PSNR: {:0.2f}, Avg srresnet PSNR: {:0.2f}" \
                      .format(e + 1, args.epochs, i + 1, len(dataloader), train_loss / (i + 1), train_psnr / total_images,
                              srresnet_psnr / total_images))

            del loss, imageLR, imageHR, imageResidual, freqLR, freqResidual, learnedResidual, learnedHR

        lrscheduler.step()

        print("Epoch [{} / {}]: Final Avg Training Loss: {:0.4f}, Final Avg Training PSNR: {:0.2f}, Final Avg srresnet PSNR: {:0.2f}" \
...
if __name__=="__main__":
    print("Beginning training for FreqSR model with srresnet...")
    args = args_parse()
    ...
    dataset = VOC2012(args.data, image_shape=(M, N), scale_factor=4, color='rgb', upsample='srresnet')
    ...
    # load srresnet model to do upsampling
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        srresnet = torch.load(f'checkpoints/srresnet/model_srresnet.pth', map_location='cpu')["model"]
    for i, (name, module) in enumerate(srresnet._modules.items()):
        module = recursion_change_bn(module)

    srresnet.to(device)
    srresnet.eval()

    train(model, srresnet, dataloader)
